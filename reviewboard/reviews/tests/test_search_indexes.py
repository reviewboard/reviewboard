...
class ReviewRequestIndexTests(TestCase):
            ...
            },
            {
                ...
                'where': Q(TupleIn(
                    ColPairs(
                        ...
                        [Repository._meta.pk],
                        [Repository._meta.pk],
                        ReviewRequest._meta.get_field('repository'),
                    ),
                    [(None,), (repository2.pk,), (repository1.pk,)],
                )),
            },
            {
                'model': User,
                ...
                'where': Q(TupleIn(
                    ColPairs(
                        ...
                        [User._meta.pk],
                        [User._meta.pk],
                        ReviewRequest._meta.get_field('submitter'),
                    ),
                    [(user3.pk,), (user2.pk,), (user1.pk,)],
                )),
            },
            {
                'model': DiffSet,
    ...
    def test_update_with_local_sites(self) -> None:
        ...
        # Repository IDs appear in the order they are first seen while
        # iterating review_requests in reverse (the queryset is ordered
        # by -last_updated).
        seen_repository_ids: list[int | None] = []

        for rr in reversed(review_requests):
            if rr.repository_id not in seen_repository_ids:
                seen_repository_ids.append(rr.repository_id)
        ...
        equeries: ExpectedQueries = [
            ...
            },
            {
                ...
                'where': Q(TupleIn(
                    ...
                    [(pk,) for pk in seen_repository_ids],
                )),
            },
            {
                'model': User,
            ...
            },
            {
                ...
                'where': Q(TupleIn(
                    ColPairs(
                        ...
                        [LocalSite._meta.pk],
                        [LocalSite._meta.pk],
                        ReviewRequest._meta.get_field('local_site'),
                    ),
                    [(None,), (local_site2.pk,), (local_site1.pk,)],
                )),
            },
        ]
