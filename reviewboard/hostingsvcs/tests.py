...
class ServiceTests(TestCase):
    ...
    def _get_repository_info(self, field, plan=None):
        if plan:
            self.assertNotEqual(self.service_class.repository_plans, None)
            result = None

            for plan_type, info in self.service_class.repository_plans:
                if plan_type == plan:
                    result = info[field]
                    break
            ...
            self.assertEqual(self.service_class.repository_plans, None)
            self.assertTrue(hasattr(self.service_class, field))

            return getattr(self.service_class, field)

    def _get_repository_form(self, plan=None, fields={}):
        form = self._get_repository_info('repository_form', plan)
    ...
    def _get_repository_fields(self, tool_name, fields, plan=None):
        form = self._get_repository_form(plan, fields)
        account = self._get_hosting_account()
        service = account.service
        self.assertNotEqual(service, None)

        return service.get_repository_fields(plan, tool_name, form.clean())
